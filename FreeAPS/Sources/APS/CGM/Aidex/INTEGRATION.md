...
```
Aidex: старт, SN=222225C99G, мастер-ключ отсутствует
...
Aidex: новая привязка — включаем нотификации F001
Aidex: -> F001 askKey
Aidex: мастер-ключ принят
Aidex: сессионный ключ принят          <- если тут CRC8, ошибка в мастер-ключе
...
- **CRC8 ключа не сошёлся** — ошибка в `makeIV`/`askKey`, либо серийный
  номер введён неверно, либо сессионный ключ расшифрован не тем ключом
  (должен использоваться мастер-ключ из нотификации F001, а не askKey)
- **«мастер-ключ принят» не появилось** — не включены нотификации F001,
  либо askKey записан не в ту характеристику
- **CRC16 пакета не сошёлся** — расшифровка идёт, но разбор неверен,
  либо не тот ключ (сессионный vs мастер-ключ)
